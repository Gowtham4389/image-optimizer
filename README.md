...
- **Optimize / convert:** real output previews and file sizes, JPG, PNG, WebP and AVIF where encoders are available, 1–100 quality, named presets, and balanced smart settings. JPEG is exported as `.jpg`.
...
The app tests **actual encoder output**, not just MIME names. Canvas can silently return PNG for unsupported requested formats; both feature detection and final export check the returned MIME type. See [MDN on canvas encoding fallback](https://developer.mozilla.org/en-US/docs/Web/API/OffscreenCanvas/convertToBlob). AVIF decoding does not imply AVIF encoding. Unsupported format choices are disabled; PNG/JPG remain available.
...
Vector artwork becomes a raster source at import time. Choose the resolution before uploading; later resizing works on those pixels. Outputs are PNG, JPG, WebP, or browser-supported AVIF, not editable SVG/EPS/PDF. SVG text uses locally available fonts; EPS fonts should be embedded, and missing fonts may be substituted. External SVG resources must be embedded first. Complex or unsupported EPS can fail with an actionable error while preserving the previous image. Print colors and spot colors are rendered to RGB.
